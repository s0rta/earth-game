(function (global, factory) {
	...
	var scaleImplicit = {name: "implicit"};
	...
	function ordinal(range) {
	  ...
	  function scale(d) {
	    ...
	    if (!i) {
	      if (unknown !== scaleImplicit) return unknown;
	...
	var interpolateHslLong = hsl$1(nogamma);
	...
	var interpolateHclLong = hcl$1(nogamma);
	...
	var utcMilliseconds = millisecond.range;
	...
	var timeTuesday = weekday(2);
	var timeWednesday = weekday(3);
	var timeThursday = weekday(4);
	var timeFriday = weekday(5);
	var timeSaturday = weekday(6);
	...
	function symbol() {
	  var type = constant$6(symbolCircle),
	      size = constant$6(64),
	      context = null;
	...
	function curveMonotoneX(context) {
	  return new MonotoneX(context);
	}

	function curveMonotoneY(context) {
	...
	function curveStepBefore(context) {
	  return new Step(context, 0);
	}

	function curveStepAfter(context) {
	...
	function stack() {
	  ...
	  stack.order = function(_) {
	    return arguments.length ? (order = _ == null ? stackOrderNone : typeof _ === "function" ? _ : constant$6(slice$3.call(_)), stack) : order;
	  };
	...
	function stackOrderInsideOut(series) {
	  var n = series.length,
	      ...
	      order = stackOrderNone(series).sort(function(a, b) { return sums[b] - sums[a]; }),
	      top = 0,
	      bottom = 0,
	      tops = [],
	      bottoms = [];
	...
	var easePolyIn = (function custom(e) {
	  e = +e;
	...
	var easePolyOut = (function custom(e) {
	  e = +e;
	...
	var easePolyInOut = (function custom(e) {
	  e = +e;
	...
	function easeBounceInOut(t) {
	  return ((t *= 2) <= 1 ? 1 - easeBounceOut(1 - t) : easeBounceOut(t - 1) + 1) / 2;
	}

	var overshoot = 1.70158;

	var easeBackIn = (function custom(s) {
	  s = +s;
	...
	var easeBackOut = (function custom(s) {
	  s = +s;
	...
	var easeBackInOut = (function custom(s) {
	  s = +s;
	...
	var amplitude = 1;
	var period = 0.3;
	var easeElasticIn = (function custom(a, p) {
	...
	var easeElasticOut = (function custom(a, p) {
	  var s = Math.asin(1 / (a = Math.max(1, a))) * (p /= tau$2);
	...
	var easeElasticInOut = (function custom(a, p) {
	  var s = Math.asin(1 / (a = Math.max(1, a))) * (p /= tau$2);
	...
	function geoCentroid(object) {
	  ...
	  return [atan2(y, x) * degrees$1, asin$1(z / sqrt(m)) * degrees$1];
	}
	...
	function circleRadius(cosRadius, point) {
	  ...
	  return ((-point[2] < 0 ? -radius : radius) + tau$4 - epsilon$3) % tau$4;
	}
	...
	function graticule() {
	  ...
	  function lines() {
	    return range(ceil(X0 / DX) * DX, X1, DX).map(X)
	        ...
	        .concat(range(ceil(x0 / dx) * dx, x1, dx).filter(function(x) { return abs(x % DX) > epsilon$3; }).map(x))
	        .concat(range(ceil(y0 / dy) * dy, y1, dy).filter(function(y) { return abs(y % DY) > epsilon$3; }).map(y));
	  }
	...
	function geoInterpolate(a, b) {
	  var x0 = a[0] * radians,
	      ...
	      kx1 = cy1 * cos$1(x1),
	      ky1 = cy1 * sin$1(x1),
	      d = 2 * asin$1(sqrt(haversin(y1 - y0) + cy0 * cy1 * haversin(x1 - x0))),
	      k = sin$1(d);
	...
	function compareIntersection(a, b) {
	  return ((a = a.x)[0] < 0 ? a[1] - halfPi$3 - epsilon$3 : halfPi$3 - a[1])
	       - ((b = b.x)[0] < 0 ? b[1] - halfPi$3 - epsilon$3 : halfPi$3 - b[1]);
	}
	...
	function clipAntimeridianLine(stream) {
	  ...
	  return {
	    ...
	    point: function(lambda1, phi1) {
	      ...
	      if (abs(delta - pi$4) < epsilon$3) { // line crosses a pole
	...
	function clipCircle(radius, delta) {
	  ...
	  function intersect(a, b, two) {
	    ...
	    var delta = lambda1 - lambda0,
	        polar = abs(delta - pi$4) < epsilon$3,
	...
	function projection(project) {
	  return geoProjectionMutator(function() { return project; })();
	}

	function geoProjectionMutator(projectAt) {
	  var project,
	      k = 150, // scale
	...
	function geoConicEqualAreaRaw(y0, y1) {
	  var sy0 = sin$1(y0), n = (sy0 + sin$1(y1)) / 2;

	  // Are the parallels symmetrical around the Equator?
	  if (abs(n) < epsilon$3) return cylindricalEqualAreaRaw(y0);
	...
	function geoConicEqualArea() {
	  return conicProjection(geoConicEqualAreaRaw)
	      .scale(155.424)
	      .center([0, 33.6442]);
	}
	...
	var geoAzimuthalEqualAreaRaw = azimuthalRaw(function(cxcy) {
	  return sqrt(2 / (1 + cxcy));
	});

	geoAzimuthalEqualAreaRaw.invert = azimuthalInvert(function(z) {
	  return 2 * asin$1(z / 2);
	});

	function geoAzimuthalEqualArea() {
	  return projection(geoAzimuthalEqualAreaRaw)
	      .scale(124.75)
	      .clipAngle(180 - 1e-3);
	}

	var geoAzimuthalEquidistantRaw = azimuthalRaw(function(c) {
	  return (c = acos(c)) && c / sin$1(c);
	});

	geoAzimuthalEquidistantRaw.invert = azimuthalInvert(function(z) {
	  return z;
	});

	function geoAzimuthalEquidistant() {
	  return projection(geoAzimuthalEquidistantRaw)
	      .scale(79.4188)
	      .clipAngle(180 - 1e-3);
	}

	function geoMercatorRaw(lambda, phi) {
	  return [lambda, log$1(tan((halfPi$3 + phi) / 2))];
	}

	geoMercatorRaw.invert = function(x, y) {
	  return [x, 2 * atan(exp(y)) - halfPi$3];
	};

	function geoMercator() {
	  return mercatorProjection(geoMercatorRaw)
	      .scale(961 / tau$4);
	}
	...
	function geoConicConformalRaw(y0, y1) {
	  var cy0 = cos$1(y0),
	      n = y0 === y1 ? sin$1(y0) : log$1(cy0 / cos$1(y1)) / log$1(tany(y1) / tany(y0)),
	      f = cy0 * pow$1(tany(y0), n) / n;

	  if (!n) return geoMercatorRaw;

	  function project(x, y) {
	    if (f > 0) { if (y < -halfPi$3 + epsilon$3) y = -halfPi$3 + epsilon$3; }
	    else { if (y > halfPi$3 - epsilon$3) y = halfPi$3 - epsilon$3; }
	...
	function geoConicConformal() {
	  return conicProjection(geoConicConformalRaw)
	      .scale(109.5)
	      .parallels([30, 30]);
	}

	function geoEquirectangularRaw(lambda, phi) {
	  return [lambda, phi];
	}

	geoEquirectangularRaw.invert = geoEquirectangularRaw;

	function geoEquirectangular() {
	  return projection(geoEquirectangularRaw)
	      .scale(152.63);
	}

	function geoConicEquidistantRaw(y0, y1) {
	  var cy0 = cos$1(y0),
	      n = y0 === y1 ? sin$1(y0) : (cy0 - cos$1(y1)) / (y1 - y0),
	      g = cy0 / n + y0;

	  if (abs(n) < epsilon$3) return geoEquirectangularRaw;
	  ...
	  project.invert = function(x, y) {
	    var gy = g - y;
	    return [atan2(x, abs(gy)) / n * sign$1(gy), g - sign$1(n) * sqrt(x * x + gy * gy)];
	  };
	...
	function geoConicEquidistant() {
	  return conicProjection(geoConicEquidistantRaw)
	      .scale(131.154)
	      .center([0, 13.9389]);
	}

	function geoGnomonicRaw(x, y) {
	  var cy = cos$1(y), k = cos$1(x) * cy;
	  return [cy * sin$1(x) / k, sin$1(y) / k];
	}

	geoGnomonicRaw.invert = azimuthalInvert(atan);

	function geoGnomonic() {
	  return projection(geoGnomonicRaw)
	      .scale(144.049)
	      .clipAngle(60);
	}

	function scaleTranslate(kx, ky, tx, ty) {
	  return kx === 1 && ky === 1 && tx === 0 && ty === 0 ? identity$6 : transformer({
	    point: function(x, y) {
	...
	function geoIdentity() {
	  var k = 1, tx = 0, ty = 0, sx = 1, sy = 1, transform = identity$6, // scale, translate and reflect
	      x0 = null, y0, x1, y1, clip = identity$6, // clip extent
	      cache,
	      cacheStream,
	      projection;
	  ...
	  return projection = {
	    ...
	    clipExtent: function(_) {
	      return arguments.length ? (clip = _ == null ? (x0 = y0 = x1 = y1 = null, identity$6) : clipExtent(x0 = +_[0][0], y0 = +_[0][1], x1 = +_[1][0], y1 = +_[1][1]), reset()) : x0 == null ? null : [[x0, y0], [x1, y1]];
	    },
	    scale: function(_) {
	...
	function geoOrthographicRaw(x, y) {
	  return [cos$1(y) * sin$1(x), sin$1(y)];
	}

	geoOrthographicRaw.invert = azimuthalInvert(asin$1);

	function geoOrthographic() {
	  return projection(geoOrthographicRaw)
	      .scale(249.5)
	      .clipAngle(90 + epsilon$3);
	}

	function geoStereographicRaw(x, y) {
	  var cy = cos$1(y), k = 1 + cos$1(x) * cy;
	  return [cy * sin$1(x) / k, sin$1(y) / k];
	}

	geoStereographicRaw.invert = azimuthalInvert(function(z) {
	  return 2 * atan(z);
	});

	function geoStereographic() {
	  return projection(geoStereographicRaw)
	      .scale(250)
	      .clipAngle(142);
	}

	function geoTransverseMercatorRaw(lambda, phi) {
	  return [log$1(tan((halfPi$3 + phi) / 2)), -lambda];
	}

	geoTransverseMercatorRaw.invert = function(x, y) {
	  return [-y, 2 * atan(exp(x)) - halfPi$3];
	};

	function geoTransverseMercator() {
	  var m = mercatorProjection(geoTransverseMercatorRaw),
	      center = m.center,
	      rotate = m.rotate;
	...
	function packEnclose(circles) {
	  ...
	  a = [b._], c = b; while ((c = c.next) !== b) a.push(c._); c = packEnclose$1(a);
	...
	function pack$1() {
	  ...
	  pack.padding = function(x) {
	    return arguments.length ? (padding = typeof x === "function" ? x : constant$10(+x), pack) : padding;
	  };
	...
	function treemap() {
	  ...
	  treemap.paddingInner = function(x) {
	    return arguments.length ? (paddingInner = typeof x === "function" ? x : constant$10(+x), treemap) : paddingInner;
	  };
	  ...
	  treemap.paddingTop = function(x) {
	    return arguments.length ? (paddingTop = typeof x === "function" ? x : constant$10(+x), treemap) : paddingTop;
	  };

	  treemap.paddingRight = function(x) {
	    return arguments.length ? (paddingRight = typeof x === "function" ? x : constant$10(+x), treemap) : paddingRight;
	  };

	  treemap.paddingBottom = function(x) {
	    return arguments.length ? (paddingBottom = typeof x === "function" ? x : constant$10(+x), treemap) : paddingBottom;
	  };

	  treemap.paddingLeft = function(x) {
	    return arguments.length ? (paddingLeft = typeof x === "function" ? x : constant$10(+x), treemap) : paddingLeft;
	  };
	...
	function randomLogNormal() {
	  var randomNormal$$ = randomNormal.apply(this, arguments);
	  return function() {
	...
	function randomBates(n) {
	  var randomIrwinHall$$ = randomIrwinHall(n);
	  return function() {
	...
	var csv$1 = dsv("text/csv", csvParse);

	var tsv$1 = dsv("text/tab-separated-values", tsvParse);
	...
	var zoomIdentity = new Transform(1, 0, 0);

	transform.prototype = Transform.prototype;

	function transform(node) {
	  return node.__zoom || zoomIdentity;
	}
	...
	function defaultTransform() {
	  return this.__zoom || zoomIdentity;
	}
	...
	var index$2 = Object.freeze({
		...
		geoCircle: geoCircle,
		geoClipExtent: geoClipExtent,
		geoDistance: geoDistance,
		geoGraticule: graticule,
		geoGraticule10: geoGraticule10,
		geoInterpolate: geoInterpolate,
		geoLength: geoLength,
		geoPath: geoPath,
		geoAlbers: geoAlbers,
		...
		geoProjectionMutator: geoProjectionMutator,
		...
		polygonContains: polygonContains$1,
		...
		randomBates: randomBates,
		randomIrwinHall: randomIrwinHall,
		...
		timeMilliseconds: utcMilliseconds,
		utcMillisecond: millisecond,
		utcMilliseconds: utcMilliseconds,
		timeSecond: second,
		timeSeconds: utcSeconds,
		utcSecond: second,
	...
	function labelnodes(width, height) {
		...
		nodedata.each(function(n,i) {
			...
			var cradius = nodecircle[i]["r"].baseVal.value
	...
	exports.foodweblayout = foodweblayout;
	exports.foodwebstatic = foodwebstatic;
	exports.labelnodes = labelnodes;
